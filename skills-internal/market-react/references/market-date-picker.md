...
```tsx
import { MarketDatePicker } from '@squareup/market-react/trial';
```
...
- **`BUILT_IN_PRESETS` constant is available** — import `{ BUILT_IN_PRESETS }` from `@squareup/market-react/trial` for type-safe preset strings (`BUILT_IN_PRESETS.TODAY`, `BUILT_IN_PRESETS.THIS_WEEK`, etc.)
