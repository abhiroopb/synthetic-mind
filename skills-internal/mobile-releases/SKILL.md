...
description: Browse, download, and install mobile app builds from mobile-releases.squareup.com (go/mr). Use when the user wants to find app versions, list available builds, download APKs or IPAs, or install builds to Android devices or iOS simulators. Covers Cash, Point of Sale, Dashboard, and all other projects on the mobile releases portal.
roles: [android, ios]
allowed-tools:
...
The `mr.py` script queries `mobile-releases.squareup.com` via its JSON API and returns structured output. All commands print JSON to stdout and errors to stderr. Input validation (app IDs, platforms, versions) is enforced in the script.
...
Example: `python3 scripts/mr.py list-versions cash android`

Returns the project's variants (build types) and all available versions, newest first. Common app IDs: `cash`, `register` (Point of Sale), `dashboard`, `banking`, `goose`, `bitkey`.
...
Example: `python3 scripts/mr.py list-builds cash android 5.40.0`
...
Older builds may be archived to S3 Glacier. These have `"archived": true` in the `list-builds` output. Archived builds cannot be downloaded directly — they require a restoration request that takes ~12 hours. If a user requests an archived build, inform them and suggest using a newer version.
