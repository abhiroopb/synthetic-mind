...
All requests go through Block's internal proxy (`dd-api-proxy`), which handles authentication automatically:
...
- `references/apm-traces.md` -- when looking up a specific trace by Speleo ID or correlating envoy logs
...
Before querying Datadog, search Block's internal monitoring knowledge base for service-specific insights:

```bash
curl -s -X POST 'https://telemetry-knowl.sqprod.co/telemetry-knowledge-store/mcp/' \
  -H 'Content-Type: application/json' \
...
- [ ] **Datadog URL** -- provide a link so the user can explore further: `https://square.datadoghq.com/{logs,rum/explorer,apm/traces,monitors/manage,dashboard/<id>,ci/pipeline-executions}?query=<encoded_query>`
