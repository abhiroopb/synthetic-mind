...
├── domains/
|       {domain}/                        # Domain-level knowledge (see go/datadomains)
│         ├── glossary.json                # Key terms and definitions
│         ├── context.txt                  # Business context for data
│         └── {subdomain}/                 # Subdomain-level knowledge (see go/datadomains)
│             ├── {topic}.txt              # Content
...
Brand knowledge stores provide foundational context for querying data related to a specific business unit (e.g., Square, Cash App, Afterpay).
...
Provides high-level context about the subdirectory's relevance and scope. This should match what exists in go/datadomains
...
4. **Expert References**
   - LDAPs of domain experts and query pattern leaders
...
✅ **DO:**
- Itemize tables and schemas clearly (e.g., `APP_PAYROLL.PAYROLL.FACT_TIMECARDS`)
- Include specific LDAP usernames of experts to follow
...
Each file contains:
- Relevant tables (e.g., `APP_STAFF.STAFF.DIM_EMPLOYEES`)
- Key columns and their usage
- Common query patterns
- Expert LDAPs to reference
