---
name: snowflake
description: Query Snowflake data warehouse using Okta SSO authentication. Run SQL queries, explore schemas, and analyze data.
metadata:
  author: jge, smorse
  version: "0.3.0"
  status: experimental
...
Query Square's Snowflake data warehouse with Okta SSO authentication.
...
Your Okta SSO identity may have admin-level access to databases you own, so destructive
...
For example:
```bash
snow sql --role CASH_DATA_BOT__SNOWFLAKE__READ_ONLY --format JSON -q "SELECT * FROM cash_data_bot.raw.my_table LIMIT 10"
```
...
By default, commands connect to **production** (`SQUAREINC-SQUARE`) with `ADHOC__MEDIUM`.
Use the `-c` flag to change the warehouse size or target **staging** (`SQUAREINC-SQUARESTAGING`):
...
Authentication uses Okta SSO via `externalbrowser`. A browser window opens automatically when authentication is
needed (first use or token expiry). If an active Okta session exists, it completes silently. Requires VPN.
...
| `Account not found` | Verify account is `SQUAREINC-SQUARE` (prod) or `SQUAREINC-SQUARESTAGING` (staging) in `~/.snowflake/config.toml` |
