# Brand A Identity Verification — ONBOARD

Query and analyze payment onboarding data from the ONBOARD Snowflake database, owned by the Payment Onboarding team in the Identity Organization at Brand A.
...
- `CUSTOMER_ID` (TEXT) — Merchant being activated
- `LOCATION_ID` (TEXT) — Unit token for the merchant
...
Tracks the activation lifecycle for a merchant. There should be **only one record per merchant+unit pair** (though most merchants have a single record per customer_id). Points to the current activation flow.

**Key Fields:**
- `ID` (NUMBER) — Primary key
- `CURRENT_ACTIVATION_FLOW_ID` (NUMBER) — FK → `ACTIVATION_FLOWS.ID` (the current/latest flow)
- `LOCATION_ID` (TEXT) — Unit token
...
**Key Fields:**
- `ID` (NUMBER), `CUSTOMER_ID` (TEXT), `IDENTITY_HASH` (TEXT), `IDENTITY_HASH_TYPE` (TEXT)
- `CREATED_AT` (TIMESTAMP_NTZ), `UPDATED_AT` (TIMESTAMP_NTZ)
...
**Key Fields:**
- `ID` (NUMBER), `LOCATION_ID` (TEXT), `CARD_BRAND` (TEXT), `FELICA_BRAND` (TEXT)
...
**Key Fields:**
- `ID` (NUMBER), `REQUEST_ID` (TEXT), `CUSTOMER_ID` (TEXT), `COUNTRY_CODE` (TEXT)
...
**Key Fields:**
- `ID` (NUMBER), `LOCATION_ID` (TEXT), `SSP_REQUEST_TOKEN` (TEXT)
...
**Key Fields:**
- `ID` (NUMBER), `LOCATION_ID` (TEXT), `PRIMARY_STORE` (NUMBER)
...
- `MERCHANT_ACTIVATION_ID` (NUMBER) — FK → `MERCHANT_ACTIVATIONS.ID`
- `CUSTOMER_ID` (TEXT), `LOCATION_ID` (TEXT)
...
**Key Fields:**
- `ID` (NUMBER), `LOCATION_ID` (TEXT), `TERMINAL_ID` (TEXT)
...
**Key Fields:**
- `ID` (NUMBER), `CUSTOMER_ID` (TEXT), `IS_ELIGIBLE` (BOOLEAN), `ANSWERS` (TEXT)
...
**Key Fields:**
- `ID` (NUMBER), `LOCATION_ID` (TEXT), `REGISTER_ALL` (BOOLEAN)
...
WHERE MA.CUSTOMER_ID = '<customer_id>'
```
...
WHERE AF.CUSTOMER_ID = '<customer_id>'
...
-- See roster-merchant-location-employee-lookups.md for ROSTER CUSTOMER_ID binary cast pattern
SELECT
    TO_VARCHAR(M.CUSTOMER_ID, 'UTF-8') AS customer_id,
    M.PRIMARY_NAME,
    AF.STATUS AS activation_status,
    AF.COUNTRY_CODE,
    UR.UNDERWRITING_STATE,
...
JOIN ONBOARD.RAW_OLTP.MERCHANT_ACTIVATIONS MA
    ON MA.CUSTOMER_ID = TO_VARCHAR(M.CUSTOMER_ID, 'UTF-8')
JOIN ONBOARD.RAW_OLTP.ACTIVATION_FLOWS AF
    ON AF.ID = MA.CURRENT_ACTIVATION_FLOW_ID
LEFT JOIN ONBOARD.RAW_OLTP.UNDERWRITING_REQUESTS UR
    ON UR.ID = AF.UNDERWRITING_REQUEST_ID
...
WHERE TO_VARCHAR(M.CUSTOMER_ID, 'UTF-8') = '<customer_id>'
