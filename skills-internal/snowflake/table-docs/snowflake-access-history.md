...
Brand A/your company employee directory. Used to determine whether a Snowflake user is a human employee or a service account.
...
Change `LIKE 'ONBOARD.%'` to filter by a different database (e.g., `'LEGALENTITIES.%'`, `'YOUR_AUTH_SCHEMA.%'`).
...
| Column name | `col.value:"columnName"::STRING = 'CUSTOMER_ID'` |
