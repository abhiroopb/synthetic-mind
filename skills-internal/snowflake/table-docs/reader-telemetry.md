...
### EVENTSTREAM2.CATALOGS.READER_TELEMETRY

One row per telemetry event emitted from the Square Register Android app for a card reader interaction.
...
|--------|-------------|---------|
| `SUBJECT_UNIT_TOKEN` | Unit token (merchant/device) — **preferred filter for oncall investigations** | `'ABC12DEFG3H4J'` |
| `SUBJECT_MERCHANT_TOKEN` | Merchant token — broader than unit token | `'XY98ABCD2EF1G'` |
...
| `HW_UNIT_TOKEN` | Unit token scoped to the HW event context | same as `SUBJECT_UNIT_TOKEN` in practice |
...
| `MOBILE_APP_PACKAGE_NAME` | Package name | `'com.squareup'` |
| `ANDROID_DEVICE_ID` | Android device ID | |
| `OS_VERSION` | Android OS version | `'14'` |

### Square Hardware (SQUID/BRAN — only populated on Square-manufactured hardware, not COTS)

| Column | Description |
|--------|-------------|
| `SQ_HARDWARE_HODOR_SQUID_VERSION` | Hodor firmware version (Square hardware only) |
...
> unless you are explicitly targeting Square hardware.
...
Source:
- `android-register/cdx/cardreaders/public/src/main/java/com/squareup/cardreaders/StateLogger.kt`
- `android-register/cdx/analytics/public/src/main/java/com/squareup/cdx/analytics/reader/CardreaderAnalytics.kt`
...
FROM EVENTSTREAM2.CATALOGS.READER_TELEMETRY
WHERE U_RECORDED_DATE BETWEEN
    ...
    AND SUBJECT_UNIT_TOKEN = '<unit_token>'
    AND HW_EVENT_NAME IN ('Workflow: State Transition', 'Workflow: State Timeout')
ORDER BY U_RECORDED_AT ASC
```

**By merchant instead:** swap `SUBJECT_UNIT_TOKEN = '...'` for `SUBJECT_MERCHANT_TOKEN = '<merchant_token>'`.
**By serial instead:** swap `SUBJECT_UNIT_TOKEN = '...'` for `HW_SERIAL_NUMBER = '<serial>'`.
