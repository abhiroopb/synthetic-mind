...
### EVENTSTREAM2.CATALOGS.MULTIPASS_EVENT
...
- `subject_merchant_token` - Merchant identifier
...
daily_stats AS (
  ...
  FROM EVENTSTREAM2.CATALOGS.MULTIPASS_EVENT m
...
FROM EVENTSTREAM2.CATALOGS.MULTIPASS_EVENT me
WHERE 1=1
...
SELECT
  ...
  me.connection_ip_address,
  me.connection_user_agent
FROM EVENTSTREAM2.CATALOGS.MULTIPASS_EVENT AS me
WHERE 1=1
  AND me.u_recorded_at >= DATEADD('month', -3, CURRENT_DATE())
  AND (
...
merchant_counts AS (
  SELECT
    COUNT(DISTINCT subject_merchant_token) as total_merchants,
...
SELECT
  ...
  COUNT(DISTINCT subject_person_token) as unique_users,
  COUNT(DISTINCT subject_merchant_token) as unique_merchants,
