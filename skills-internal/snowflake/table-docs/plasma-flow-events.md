...
drives every user-facing step in Cash App — passcode entry, instrument selection, confirm-recipient screens, IDV,
scam warnings, and more. This table records every event emitted by Plasma flows across the entire app, not just P2P.

## Tables

### APP_CASH.CASH_DATA_BOT.STAGING_PLASMA_FLOW_EVENTS (preferred)
...
(defined in `plasma/plasma-events/src/main/proto/squareup/cash/plasma/flows/events.proto`). The publisher sets
...
| `GUARDRAILS_ADVERSITY_BLOCKED_ACTION_REQUIREMENT` | ~98K | Blocked by guardrails adversity |
