...
| `order_debit_card` | bool | `false` | Order a Cash Card |
...
| `borrow_credit_line` | bool | `false` | Enable Borrow eligibility. Requires: `complete_idv`, `set_address`, `order_debit_card`, and a supported `us_state` (e.g. `"TX"`, `"CA"`, `"NY"`) |
...
| `retro_credit_line` | bool | `false` | Enable Retro eligibility. Requires: `complete_idv`, `set_address`, `order_debit_card`, `link_phone_number`, a supported `us_state` (e.g. `"TX"`), and `cash-retro-testers` LD segment. Post-creation: approve/activate card, add credit decision via Toolbox Prospector |
...
| `bnpl_credit_line` | bool | `false` | Enable BNPL on Debit Card eligibility. Requires: `complete_idv`, `set_address`, `order_debit_card`, and a supported `us_state`. Post-creation: approve/activate card, add `bnpl-card-users` LD segment, add credit decision via Toolbox Prospector with `PRE_PURCHASE_FINANCING` |
...
| `bnpl_credit_line_amount_cents` | int | `40000` | APCAC credit line amount in cents (only used when `bnpl_credit_line: true`) |
