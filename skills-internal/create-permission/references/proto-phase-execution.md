...
File: `~/Development/java/auth/auth-protos/src/main/proto/permissions.proto`
...
File: `~/Development/java/shared-protos/employees/src/main/proto/employees/employees.proto`
...
> I'm about to update the auth certification test file (`logged_in_user_certification.json`). This is expected and not something you did wrong — every time a new employee enum is added, this file's byte arrays need to be updated to include the new permission bit. I'll run the certification test afterward to verify the update is correct.

When new employee permission enums are added, the auth certification test file must be updated. The file contains example users with byte-array-encoded permissions, and some users are expected to have ALL employee permissions set.

File: `~/Development/java/auth/client/src/test/resources/certification/logged_in_user_certification.json`
...
```bash
bazel test //auth/client/src/test/java/com/auth/client:all_tests_all_shards --test_filter=com.auth.client.AuthUserTest#exampleUsersWithAllEmployeePermissions
```
...
File: `~/Development/go/src/app/up/oauth/protos/oauth/v1/oauth-permission.proto`
...
- The `enum_value_version` must be a valid API release train date (gathered in Stage 2), not the current date or commit date
- Add doc comment with HTTP methods

After editing the proto, the generated Go code (`go-protos/oauth/v1/oauth-permission.pb.go`) must be regenerated. The generation script can take several minutes on a cold machine (building protoc plugins, running `go mod vendor`), so hand it off to the user:
...
> ./script/generate-proto ./oauth/protos/oauth/v1/oauth-permission.proto
> ```
>
...
File: `~/Development/java/auth/common/src/main/resources/permissions-map.yaml`

- Under `permissions.MAP`, find the correct alphabetical position
...
For the Java monorepo (auth proto and/or Employee proto and/or permissions map and/or certification JSON):
```bash
cd ~/Development/java
git checkout -b add-permission-PERMISSION_NAME
git add -A
git commit -m "Add PERMISSION_NAME permission"
gh pr create --title "Add PERMISSION_NAME permission" --body-file /tmp/pr-body.txt --reviewer "<org>/permissions-team"
```
...
gh pr create --title "Add PERMISSION_NAME OAuth permission" --body-file /tmp/pr-body.txt --reviewer "<org>/permissions-team"
```
