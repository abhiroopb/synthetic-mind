...
**AddDeveloperEnum**
- Adds a new enum to `RequestPermissionFlags` in the Multipass proto.
- Required: `developer_permission.name`, `developer_permission.description`
- Repo: `~/Development/java`
- File: `multipass/multipass-protos/src/main/proto/squareup/multipass/permissions.proto`
...
- Repo: `~/Development/java`
- File: `shared-protos/employees/src/main/proto/squareup/employees/employees.proto`

**UpdateCertificationJSON**
- A required mechanical side-effect of adding an employee enum. The Multipass certification test file contains example users that must have every employee permission bit set, so adding a new enum means updating byte arrays in that file. This is not something the user did wrong — it's expected every time an employee enum is added. Always co-occurs with AddEmployeeEnum.
- Required: employee enum values (derived from AddEmployeeEnum execution)
- Repo: `~/Development/java`
- File: `multipass/client/src/test/resources/certification/logged_in_user_certification.json`

**AddOAuthPermission**
- Adds a new enum to `OAuthPermission` in the OAuth proto.
- Required: `developer_permission.name`, `developer_permission.oauth.*`
- Repo: `~/Development/go/src/square/up`
- File: `oauth/protos/squareup/oauth/v1/oauth-permission.proto`

**AddOrUpdateSafeMapping**
- Adds or modifies an entry in the Envoy SAFE permissions map.
- Required: `developer_permission.safe_mapping.*`
- Repo: `~/Development/java`
- File: `multipass/common/src/main/resources/multipass-permissions-map.yaml`
...
- Repos: `~/Development/java`, `~/Development/go/src/square/up`, `~/Development/roster-rails`, `~/Development/dashboard`

**AddOrUpdateDisplayPermission**
- Adds or modifies a display permission config in em-permissions YAML, including locale entries. Handles all three backing modes: `employee_permission: true` (simple shorthand), `employee_permission_mapping` (per-access-point enums), and UI-only (no employee backing).
...
- Repo: `~/Development/em-permissions`
