...
SEGMENT_JSON=$(sq curl -s "${BASE_URL}/api/v1/segments?id=${SEGMENT_ID}")
...
CSRF_TOKEN=$(sq curl -s -c "$COOKIE_JAR" "${BASE_URL}/ui/segments/${SEGMENT_ID}" \
  | grep 'csrf-token' \
  | sed 's/.*content="\([^"]*\)".*/\1/')
...
RESPONSE=$(sq curl -s -X PATCH "${BASE_URL}/api/v1/segments" \
  -b "$COOKIE_JAR" \
