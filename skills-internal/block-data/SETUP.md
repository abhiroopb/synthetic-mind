# Block Data Skill Setup

## Prerequisites

- **VPN connected** (required for Snowflake Okta SSO authentication)
- **`SNOWFLAKE_USER`** set to your Square email (e.g., `user@squareup.com`)
...
```bash
claude mcp add --scope user -e SNOWFLAKE_USER=your_email@squareup.com mcp-block-data -- uvx mcp_block_data@latest
```
...
4. Run your query (e.g., "What's Square's GPV?")
...
| `SNOWFLAKE_USER not set` | Add `export SNOWFLAKE_USER="email@squareup.com"` to `~/.zshrc` |
