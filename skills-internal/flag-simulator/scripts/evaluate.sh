...
# CLI replacement for go/YOUR_INTERNAL_LINK (console.internal.example.com/flag-simulator)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help)
      ...
      echo "  --project PROJECT         PIE|CAPITAL|SQUARE_CONSOLE|Brand D|SHOP (default: PIE)"
...
get_domain() {
  case "$1" in
    production) echo "experiment-relay.internal.example.com" ;;
    staging)    echo "experiment-relay.stage.internal.example.com" ;;
    sandbox)    echo "sandbox--experiment-relay.internal.example.com" ;;
...
get_bundle_id() {
  case "$1" in
    SPOS)            echo "com.example.app" ;;
    Invoices)        echo "com.example.invoices" ;;
    Appointments)    echo "com.example.appointments" ;;
    Restaurants)     echo "com.example.restaurant" ;;
    Retail)          echo "com.example.retailer" ;;
    Dashboard)       echo "com.example.dashboard" ;;
    "Terminal API")  echo "com.example.terminalapi" ;;
    Kiosk)           echo "com.example.fnbkiosk" ;;
    KDS)             echo "com.example.kds" ;;
    "Squid Settings") echo "com.example.devicesettings" ;;
    "Local Connect") echo "com.example.localconnect" ;;
    OOB)             echo "com.example.oob" ;;
    Team)            echo "com.example.payroll" ;;
...
get_device_string() {
  case "$1" in
    X2A)  echo "DeviceMfg DeviceMfg ModelA on msm8916_64 SQUID" ;;
    X2B)  echo "DeviceMfg DeviceMfg ModelA on sdm660_64 SQUID" ;;
    X2C)  echo "Brand A Brand A X2c SQUID" ;;
    T2A)  echo "Brand A Brand A T2 on msm8916_64 SQUID" ;;
    T2B)  echo "Brand A Brand A T2 on sdm660_64 SQUID" ;;
    T3A)  echo "Brand A Brand A T3a SQUID" ;;
