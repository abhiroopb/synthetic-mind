...
description: Add or remove tokens (serial numbers, UUIDs) from Tarkin segments. Use when asked to modify segment membership, add devices to a segment, or remove devices from a segment.
allowed-tools:
...
# Tarkin Segment Tokens

Add or remove tokens from Tarkin segments while preserving segment metadata.
...
- Check `data` array length to determine if the segment was found
...
- Handles CSRF authentication via cookie jar
...
Segment 50: "moira test t2b" (SerialNumberSegment), 1 item

Agent: Please confirm:
- Environment: staging
- Segment: #50 "moira test t2b" (1 current item)
...
For bulk operations with multiple product types or EFFA segments, read `scripts/BULK-WORKFLOW.md` for the full workflow and naming conventions.
...
- Staging uses `tarkin.stage.sqprod.co`, production uses `tarkin.sqprod.co`
- Authentication is handled via `sq curl` (BeyondCorp)
