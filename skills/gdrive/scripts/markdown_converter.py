...
class MarkdownToDocsConverter:
    ...
    def __init__(self, base_index: int = 1):
    ...
    def _generate_requests(self) -> dict:
        ...
        # 4. Apply text formatting - descending order
    ...
    def generate_formatting_requests(self, doc_body: list[dict]) -> dict:
        ...
        for start, _end, content in sorted(text_positions, key=lambda x: x[0]):
            for i, char in enumerate(content):
                doc_pos_to_actual[len(doc_text)] = start + i
                doc_text += char
        ...
        def find_text_position(text: str, start_after: int = 0) -> tuple[int, int] | None:
        ...
        # Apply text formatting by searching for content
        ...
        # Apply paragraph styles (headings) - search for heading text
        for pr in self.paragraph_ranges:
            if pr.style_type == "NORMAL_TEXT":
                continue

            # Get the heading text from our original text (using UTF-16 positions)
            heading_text = self._slice_text_by_utf16(pr.start, pr.end).strip()
            if not heading_text:
                continue

            pos_range = find_text_position(heading_text)
            if pos_range is None:
                continue
    ...
    def _generate_table_cell_requests(self, table: TableSpec, table_index: int) -> list[dict]:
        ...
        1. Reset paragraph style to NORMAL_TEXT
        2. Insert text content (if any)
        3. Apply bold formatting for header row
        """
        requests = []
        rows = table.rows
        cols = table.cols
        ...
        for r in range(rows - 1, -1, -1):
            for c in range(cols - 1, -1, -1):
                ...
                # 1. Reset cell to NORMAL_TEXT to prevent inheriting heading styles
                requests.append({
                    "updateParagraphStyle": {
                        "range": {
                ...
                if cell_text:
                    ...
                    # 3. Bold header row cells immediately after insert
                    if r == 0:
                        requests.append({
                            "updateTextStyle": {
                                "range": {
                                    "startIndex": cell_index,
                                    "endIndex": cell_index + MarkdownToDocsConverter._utf16_len(cell_text),
                                },
                                "textStyle": {"bold": True},
                                "fields": "bold",
                            }
...
def convert_markdown_to_docs_requests(markdown: str, base_index: int = 1) -> dict:
