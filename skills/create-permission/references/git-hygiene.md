...
For the Proto Phase, always reset `~/Development/java` upfront. If the plan includes OAuth operations, also reset `~/Development/go/src/square/up`. If the plan includes display permission operations, also reset `~/Development/em-permissions`.

For the Config Phase, reset ALL repos: `java`, `go/src/square/up`, `em-permissions`, `roster-rails`, `dashboard`.
