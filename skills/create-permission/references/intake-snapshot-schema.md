...
If the user hasn't submitted an intake form or doesn't have a URL, tell them: "This skill requires an approved intake form from the Permissions Working Group before proceeding with implementation. You can submit one via the permissions intake form. Once it's been reviewed and approved, come back with the Notion URL and we'll get started." Do NOT proceed without it.
...
- Parse the page content into the snapshot schema below. Look for:
  ...
  - Which components are needed (mentions of developer permission, employee permission, display permission, OAuth, API gateway auth, team permissions UI, employee_permission_mapping)
...
developer_permission:
  ...
  oauth:
    ...
    version: API release train date
  safe_mapping:
    needed: yes/no
