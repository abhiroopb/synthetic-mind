...
The Config Phase is only needed when proto changes were made in the Proto Phase. If the state file shows no proto changes, there is no Config Phase — the em-permissions work was already completed in the single-pass flow.
...
After reading the state file, echo back the key values to the user and ask them to confirm before proceeding. Display: which operations are planned, permission names, Proto Phase PR URLs, and for display permissions the em-permissions details (group, access points, parent, display name, backing mode, per-access-point mapping if applicable). Do NOT proceed until the user confirms. This catches any accidental edits or file corruption between phases.

Then verify that the saved state is still valid against the current codebase. Check ALL of the following:

- **Permission group exists** (if display permission in plan): confirm the group still exists in `~/Development/em-permissions/config/permissions/permission_group_ordering.yml` (unless it's a new group).
...
- **Permission set templates are valid** (if templates are specified): confirm those template IDs exist in `~/Development/em-permissions/config/permissions/permission_set_templates.yml`.
- **No naming conflicts**: confirm the permission names/IDs don't already appear in the relevant protos or em-permissions YAML configs (another change may have landed between phases).
...
- **VPN**: Run `dig +short dns.sip.west.us.square` and check that it returns an IP address. If it returns nothing, tell the user: "VPN doesn't appear to be connected. The all-protos bump commands need access to internal package registries. Please connect to VPN and tell me when you're ready."
- **Ruby repos** (roster-rails, dashboard, em-permissions): Non-interactive shells don't load Ruby version managers automatically. Before ANY Ruby or Bundler command in the Config Phase, initialize the user's Ruby version manager. Detect which one is available by checking in this order:
...
git add -A
git diff --quiet --cached && echo "NO_CHANGES" || (git commit -q -m "Bump all-protos for PERMISSION_NAME" && gh pr create --title "Bump all-protos for PERMISSION_NAME" --body-file /tmp/pr-body.txt --reviewer "squareup/permissions-team")
```
...
This goes in the SAME branch/PR as the em-permissions all-protos bump (if applicable).

Initialize the user's Ruby version manager, then:
```bash
cd ~/Development/em-permissions
...
```bash
rg "employee_permission: true" ~/Development/em-permissions/config/permissions/ -l
rg "employee_permission_mapping:" ~/Development/em-permissions/config/permissions/ -l
rg "access_points: all_access_points" ~/Development/em-permissions/config/permissions/
```
...
Edit the appropriate group file in `~/Development/em-permissions/config/permissions/GROUP.yml`:
...
Edit `~/Development/em-permissions/config/locales/permissions.en.yml`:
...
If the user chose Standard or Enhanced, edit `~/Development/em-permissions/config/permissions/permission_set_templates.yml`:
...
If no em-permissions changes were made (no display permission or template operations in the plan), skip this step.
...
gh pr create --title "Add PERMISSION_NAME permission to em-permissions" --body-file /tmp/pr-body.txt --reviewer "squareup/permissions-team"
```
...
All backfill scripts live in `~/Development/em-permissions/script/` and take a token file in the format `MERCHANT_TOKEN PERMISSION_SET_TOKEN1,PERMISSION_SET_TOKEN2,...` (one line per merchant). The merchant token is used for lookup, but updates happen at the permission set level. Token files can be generated with `generate_tokens_for_backfill.rb`.
...
Tell the user: "Backfill scripts need to be run after your em-permissions PR merges and deploys. You'll need to generate a token file with `generate_tokens_for_backfill.rb` and coordinate with your team on rate limiting. Reach out in #permissions if you need help with the backfill."
...
>
> These PRs can be merged in any order once CI passes. The em-permissions PR includes both the all-protos bump and the config changes, so there's no ordering dependency there.
>
...
>
> 1. `cd ~/Development/go/src/square/up`
...
1. If an employee permission was added, copy the updated certification JSON from the Java monorepo to the Go monorepo:
   ```bash
   cp ~/Development/java/multipass/client/src/test/resources/certification/logged_in_user_certification.json ~/Development/go/src/square/up/client/multipass/resources/certification/logged_in_user_certification.json
   ```
...
3. If an employee permission was added, verify the Go certification test passes:
   ```bash
   cd ~/Development/go/src/square/up
   go test ./client/multipass/ -run TestExampleUsersWithAllEmployeePermissions
   ```
...
4. Commit and open the PR:
   ...
   **IMPORTANT: Check Test Mode before creating this PR.** If test mode is active, add `--draft` and remove `--reviewer` from the `gh pr create` command.
   ...
   gh pr create --title "Bump all-protos for PERMISSION_NAME" --body-file /tmp/pr-body.txt --reviewer "squareup/permissions-team"
   ```
...
> - Test Envoy SAFE authorization with the new permission (if developer permission with SAFE mapping was created)
