...
3. Follow the **Git Hygiene** procedure for `~/Development/em-permissions` before executing.
4. Save state as usual, noting that there are no Proto Phase PRs and no Config Phase re-invocation needed.
5. Replace the closing message with: "No proto changes were needed, so there's no Config Phase to come back for. Your em-permissions PR is ready for review and can be merged once CI passes and the Permissions team approves."
