"""OAuth2 authentication for Google Calendar API with file-based credential storage."""
...
# Default paths
CONFIG_DIR = Path.home() / ".config" / "gcal-skill"
...
# Bundled OAuth client config (same GCP project as gdrive skill).
# The client_secret below is intentionally included in source code. Per Google's
# OAuth2 documentation for "installed application" (desktop/CLI) clients, the
# client secret is not treated as confidential—it is expected to be embedded
# in the distributed application. Security relies on the per-user authorization
# code flow, not on this secret.
...
def require_auth() -> Credentials:
    ...
    if not creds:
        raise RuntimeError(
            "Not authenticated. Run 'gcal-cli.py auth login' first."
        )
    return creds
