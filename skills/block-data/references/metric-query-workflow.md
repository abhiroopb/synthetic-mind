...
Complete 7-step protocol for querying metrics from the Block Metric Store.
...
- **Brand hints** — Look for mentions of Square, Cash App, Afterpay, Block, TIDAL, Bitkey. If found, include in the `brand` parameter. Valid brands: `square`, `cash_app`, `block`, `afterpay`, `tidal`, `bitkey`.
- **Domain hints** — If the user mentions a specific business domain, note it for the `domains` parameter.
- **Keywords** — Break the question into comma-delimited search keywords (e.g., "What is Cash App revenue?" → `"revenue,cash app"`).
...
1. Call `mcp__mcp-block-data__list_available_domains`
...
3. Wait for response
4. Call `mcp__mcp-block-data__list_available_metrics` with or without `domains` parameter
...
Call `mcp__mcp-block-data__metric_store_search` with:
...
- Present two options:
  1. "Submit a request for the Block Data MCP team to add this metric"
...
Call `mcp__mcp-block-data__get_metric_details` with the selected metric name.
...
**ALWAYS call `mcp__mcp-block-data__get_dimension_values` before applying filters.**
...
Call `mcp__mcp-block-data__fetch_metric_data` with:
...
2. Present two options:
   - **Option 1**: "Submit a request for the Block Data MCP team to add this metric"
...
- Call `mcp__mcp-block-data__dashboard_search` with original search terms
