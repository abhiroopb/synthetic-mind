...
- `mcp__mcp-block-data__metric_store_search` → mcp_block_data server

### Check logic:
```
Look at your tool list.
If mcp__mcp-block-data__metric_store_search is present → Server is available ✅
...
1. **Display to user:**
   ```
   ⏳ The mcp_block_data server appears to be initializing, waiting 10 seconds...
   ```
...
After waiting, the `mcp_block_data` server is still unavailable.
...
- **VPN connected**
- **`SNOWFLAKE_USER` set** to the user's Square email (e.g., `user@squareup.com`)

Ask the user for their Square email before running the install command.

### If user selects Option A (Auto-install):

Ask for their Square email, then run:
```bash
claude mcp add --scope user -e SNOWFLAKE_USER=USER_EMAIL@squareup.com mcp-block-data -- uvx mcp_block_data@latest
```
...
```json
{
  "type": "stdio",
  "command": "uvx",
  "args": ["mcp_block_data@latest"],
  "env": { "SNOWFLAKE_USER": "USER_EMAIL@squareup.com" }
}
```
...
Ask for their Square email, then display:
```
...
"mcp-block-data": {
  ...
  "command": "uvx",
  "args": ["mcp_block_data@latest"],
  "env": { "SNOWFLAKE_USER": "YOUR_EMAIL@squareup.com" }
}
```
