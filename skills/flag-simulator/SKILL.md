---
name: flag-simulator
description: "Evaluate LaunchDarkly feature flags via the Experiment Relay API. CLI replacement for go/flag-simulator. Use when user asks to evaluate a flag, check flag value, simulate flag evaluation, test flag targeting, look up flag result, query flag state, resolve flag variation, inspect flag output, or mentions flag-simulator."
...
Evaluate LaunchDarkly feature flags via the Experiment Relay API. This is a CLI replacement for [go/flag-simulator](https://go.sqprod.co/flag-simulator) (Square Console's flag simulator UI).

**STOP** and ask the user if: the flag key is unknown or ambiguous, the token (merchant/unit/person) is missing, or the platform (Server, Mobile, Dashboard Web) is not specified.
...
scripts/evaluate.sh \
  --flag-key <flag_key> \
  --token <token_value> \
  [--token-type <MERCHANT|UNIT|PERSON|SQUARE_MOBILE_DEVICE>] \
  [--project <PIE|CAPITAL|SQUARE_CONSOLE|TIDAL|SHOP>] \
  [--env <production|staging|sandbox>] \
  [--flag-type <BOOLEAN|STRING|INTEGER|DOUBLE|JSON>] \
  [--platform <Server|Mobile|"Dashboard Web">] \
  [--app-vertical <SPOS|Invoices|Appointments|Restaurants|Retail|Dashboard|...>] \
  [--app-version <version>] \
  ...
  [--context <type:token>] \
  [--default-context-type <MERCHANT|UNIT|PERSON|SQUARE_MOBILE_DEVICE>]
```
...
> "What's the value of `waiter-orders-mode` for merchant MLN7XZDSEWET1?"

```bash
scripts/evaluate.sh --flag-key waiter-orders-mode --token MLN7XZDSEWET1
```

Defaults apply: project=PIE, env=production, type=BOOLEAN, token-type=MERCHANT. Platform must be specified.

### Specifying project and environment

> "Check `capital-feature` for merchant ABC123 in staging on the CAPITAL project"
...
> "Check `ios-feature` for merchant M123 on iOS SPOS version 6.95"
...
### Mobile platform (Android/Squid)

> "Evaluate `squid-feature` on an X2C device running Restaurants 6.90"
...
> "Check `dashboard-feature` for merchant M123 from Dashboard Web"
...
> "Evaluate `ab-test` for merchant M123 with attribute country=US and tier (int) = 3"
...
> "Check `cross-context-flag` for merchant M123 and person P456"
...
- For non-Mobile platforms, a fake SQUARE_MOBILE_DEVICE context is auto-added to carry user_attributes (this matches Console UI behavior)
...
- CSRF is bypassed via the Origin header (safe because we're authenticated via VPN, not a browser session)
