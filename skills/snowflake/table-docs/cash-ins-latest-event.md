...
### APP_CASH.MONETA.CASH_INS_LATEST_EVENT

One row per cash-in transfer reflecting its latest/current state. A "cash-in" is any flow that pulls money from an
external source (debit card, credit card, bank account, Apple Pay, Google Pay) into a customer's Cash App balance. This
...
**Source code:** Proto definitions live in `moneta/client-protos/src/main/proto/squareup/cash/moneta/`. The Snowflake
...
- `APP_CASH.MONETA.CASH_INS_EVENTS` -- one row per event (multiple per transfer), ETL runs twice daily
- `APP_CASH.MONETA.CASH_INS_SUMMARY` -- aggregation layer between this table and TRANSFER_SUMMARY
- `APP_CASH.APP.TRANSFER_SUMMARY` -- combined cash-in and cash-out data (all products, not just Moneta)
...
| 84 | SQUARE_DEVICE_ID | TEXT | Y | | Square device ID (rare) | |
...
FROM APP_CASH.MONETA.CASH_INS_LATEST_EVENT ci
...
FROM APP_CASH.MONETA.CASH_INS_LATEST_EVENT ci
JOIN APP_CASH.CASH_DATA_BOT.MONEY_MOVER_TRANSACTIONS mm
...
```sql
SELECT *
FROM APP_CASH.MONETA.CASH_INS_LATEST_EVENT
