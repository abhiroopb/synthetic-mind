---
name: navigation
description: Add or update links in the Dashboard navigation menu. Use when the user wants to add a new nav link, rename a nav link, reorder nav items, or modify the sidebar navigation in Square Dashboard. Requires changes in both the dashboard and ecosystem-header repositories.
metadata:
  status: beta
---
...
This skill guides you through adding or updating navigation links in the Square Dashboard sidebar. Navigation changes require coordinated modifications across two separate repositories: **dashboard** and **ecosystem-header**.
...
- `dashboard`: <https://github.com/squareup/dashboard>
- `ecosystem-header`: <https://github.com/squareup/ecosystem-header>

Do not attempt any navigation changes until you have confirmed paths to both repositories.

## How Navigation Works

Navigation in Dashboard uses a double allow-list system. The "Current State" section of [this doc](https://github.com/squareup/square-web/blob/0c5cfd572567167de5bac87a3a64951be4150eb6/libs/shared/ui-nav-sidebar/docs/nav-vision.md#the-double-allow-list-problem) explains the architecture. Only the "Current State" section is reliable — disregard the rest of that document as it is outdated and partially inaccurate.
...
```yaml
banking.transfers:
  route_name: square-card.activity.transfers
  translation_key: nav.banking.transfers
  link: "/dashboard/square-card/activity/transfers"
...
**IMPORTANT: You MUST inform the user about these manual testing steps before finishing your work.** You (the agent) cannot test these changes end-to-end — the user must perform the verification themselves using Playpen and Requestly. Do not skip or abbreviate this section. Always walk the user through the full testing process.

### Locale string caveat

New translation strings added to `en.module.js` will **not** load on Playpen until they have been deployed to staging. Even if your local changes include the new strings, Playpen fetches translations from the deployed staging build — not from your branch.

There are two ways to handle this:

1. **Deploy locale strings first (recommended):** Open a separate dashboard PR containing _only_ your `en.module.js` changes. Merge it so the strings get deployed to staging. Once deployed, you can test the rest of your navigation changes on Playpen with the real locale strings.
2. **Use existing locale strings temporarily:** For the purpose of validating your nav changes on Playpen, temporarily point your nav item's `translation_key`, `title`, and `description` at existing locale strings that are already deployed. Once you've confirmed the nav structure works, swap in your new locale strings before merging your final PR.

### 3a. Verify the dashboard header response

First, deploy the dashboard branch to Playpen to confirm the `/dashboard/header` API response includes the new nav item. See [Playpen instructions](https://dev-guides.sqprod.co/square/docs/develop/web/dashboard/legacy/getting-started/playpen).

1. Deploy the dashboard branch to Playpen.
...
Do not proceed to the next step until the header response looks correct.

### 3b. Verify the nav renders correctly

Once the header response is confirmed, test that ecosystem-header renders the new link. Point Staging at your Playpen target for the "dashboard" service, then use Requestly to load your local ecosystem-header code:

1. Install the [Requestly Chrome Plugin](https://requestly.io/).
2. Create a new redirect request rule (e.g., name it "ecosystem-header redirect").
3. Under "Rule Conditions", set the request URL to: `https://staging.ecosystem-header.squarecdn.com/bundle.js`
4. Set the redirection URL to: `http://localhost:5001/ecosystem-header.ts`
5. Run ecosystem-header locally with `pnpm dev`.
6. Load Staging in your browser — it will fetch the header response from your Playpen dashboard and render nav using your local ecosystem-header.
7. Confirm the new nav link appears in the correct position under the correct parent.
8. Note: you must manually refresh the page to see changes.
...
4. Merge the ecosystem-header PR to get the rendering into staging for testing.
5. Verify end-to-end after ecosystem-header is promoted to production.

## Important Context

There is ongoing work in the **square-web** project to simplify navigation development and consolidate it into a single place. However, that work is not expected to be ready until approximately **May 2026**. Until then, the two-repository approach described above is the correct process.

## Reference PRs

These PRs added a "Transfers" link under Banking and serve as a complete example of the process:

- **dashboard**: [squareup/dashboard#88706](https://github.com/squareup/dashboard/pull/88706)
- **ecosystem-header**: [squareup/ecosystem-header#3613](https://github.com/squareup/ecosystem-header/pull/3613)
