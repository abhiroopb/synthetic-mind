...
Query employee data from your organization's internal People/Directory API.
...
Most endpoints also work without a token (VPN network access is sufficient), but using a token is recommended for reliability.
...
**Response fields:** `username`, `email`, `preferred_first_name`, `last_name`, `lead` (manager's username), `start_date`, `job_family`, `tech_lead`, `city`, `country_code`, `business_unit`, `slack_username`, `slack_id`, `github`, `phone_mobile`, `personal_pronoun`, `contingent` (contractor flag), `discipline`, `primary_photo`, `org_chart_nodes`.
...
Returns everything from the list endpoint plus: `bio`, `linkedin`, `timezone`, `cost_center`, `workday_id`, `manager` (boolean), `business_unit_name`, `preferred_pronunciation`, `phone_desk`, `phone_desk_extension`, `org_l1`/`org_l2`/`org_l3`, `function_l1`/`function_l2`/`function_l3`, `photos`, `photos_accessible`, `executive_assistants`.
...
Response fields: `name`, `id`, `parent_id`, `node_type`, `employee_username`, `sub_team_or_sub_function`, `reference_id`, `slack_channel`, `slack_channel_url`, `google_group`, `office_hours`, `preferred_contact_method`.
...
The `sub_team_or_sub_function` field contains the custom sub-team label set by the manager in the org chart UI. It is `null` for ungrouped ICs.
...
1. **Pagination**: List endpoint returns 500/page. Use `X-Pages` header to know when to stop.
...
4. **Profile page**: `https://directory.example.com/profile/:username`
