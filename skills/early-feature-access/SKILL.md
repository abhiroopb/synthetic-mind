...
This skill automates adding a new feature to the Early Feature Access (EFA) system in a web dashboard application.
...
1. **Ensure the web app repo is available.** If it is not cloned locally, clone it:
   ```bash
   git clone git@github.com:your-org/web-app.git
   cd web-app
   ```
...
3. **Feature gating** — e.g. `Feature.RETAIL` (optional per bullet)
...
Read the file at the content index (e.g., `libs/routes-device-management/src/early-feature-access-content/index.ts`) to get the current list of registered features. Print the list to the user in order and ask where the new feature should appear. For example:
...
### Step 3: Implement Web App Changes
...
All paths are relative to `libs/routes-device-management/src/`.
...
Follow the pattern in the existing features. Read reference files for examples of simple configs, configs with images, and type definitions.
...
**Without images:**
```typescript
import { EarlyFeatureAccessScope } from '@your-org/shared-types-protos/cls/fields/earlyfeatureaccess/early_feature_access.js';
import { ClsField } from '@your-org/shared-types-protos/cls/model/cls_field';
import { Scope } from '@your-org/shared-types-protos/settingshub/model/scope';
import type { EarlyAccessFeature } from '../types';
...
3. List of files changed/created
4. A checklist:
   - [ ] Tested in staging: `https://app.staging.example.com/dashboard/early-feature-access`
...
- Design files: Search "Early Feature Access" in your design tool
