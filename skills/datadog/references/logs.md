...
```bash
curl -s -X POST "https://dd-api-proxy.stage.sqprod.co/datadog/api/v2/logs/events/search" \
...
```bash
curl -s -X POST "https://dd-api-proxy.stage.sqprod.co/datadog/api/v2/events/search" \
  -H 'Content-Type: application/json' \
