...
curl -s -G "https://dd-api-proxy.stage.sqprod.co/datadog/api/v1/query" \
...
```
https://square.datadoghq.com/metric/explorer?fromUser=true&start=<FROM_MS>&end=<TO_MS>&paused=true#<URL_ENCODED_QUERY>
```
...
```bash
# By service
curl -s "https://dd-api-proxy.stage.sqprod.co/datadog/api/v2/metrics?filter[configured]=true&filter[tags_configured]=service:my-service&window[seconds]=3600" \
  | jq '[.data[] | .id]'
```
...
```bash
curl -s "https://dd-api-proxy.stage.sqprod.co/datadog/api/v2/metrics?filter[configured]=true&filter[tags_configured]=service:my-service,span.kind:*&window[seconds]=3600" \
  | jq '[.data[] | .id]'
```
...
```bash
curl -s "https://dd-api-proxy.stage.sqprod.co/datadog/api/v2/metrics/system.cpu.user/all-tags" \
