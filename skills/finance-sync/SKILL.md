...
1. **Statement Pre-Processing:**
   ...
   - Extract the closing balances from these PDFs.
   - Write the extracted **INR** balances to **Column I** and **SGD** balances to **Column J** of the Finances `TOPLINE` tab.
2. **Read TOPLINE tab** from Finances spreadsheet (`TOPLINE!A1:K80`) to get current balances in column K.
3. **Match accounts** using the mapping table above — match by account name (col A) + account number (col B).
4. **Insert new rows** at the top of Balance History (after header) using `sheets batch-update` with `insertDimension` on sheetId `1531277441`.
5. **Write rows** with current date/time, account metadata, and the balance from the Finances sheet.
6. **Show summary table** to the user with: Account, Old Balance (from last Balance History entry), New Balance, Change.
...
### Mazda CX-5 (Car)
- **Source:** Kelly Blue Book (KBB) Fair Purchase Price — VIN: `JM3KFBDMXJ0335582` (2018 Grand Touring)
- **Finances Cell:** `TOPLINE!H57`
- **Tiller Account:** Mazda CX-5, Acct # `8CVW950`, Account ID `manual:5bd19a8b-184d-4c64-812d-c6da20b87cd8`, Institution: Car, Type: Other, Class: Asset

### 2424 Tulare Ave (House)
- **Source:** Zillow Zestimate
- **Finances Cell:** `TOPLINE!H53`
- **Tiller Account:** 2424 Tulare Ave, Account ID `manual:04feacf4-7d42-4e52-9505-cc02bcb6a471`, Institution: Home, Type: Other, Class: Asset
- **Also update:** [Payments spreadsheet](https://docs.google.com/spreadsheets/d/1hu0tTq4xCq_O6HrHaDV8dWLi9gGUBQoEbg2xHC3jlvU) → `Mortgage (Refi)` tab → Column K ("Est. Home Value") at the row for the current month

## Sync Completion

After all updates, write today's date into `TOPLINE!T9` to mark the sync completion.
